...
class VideoSwinTransformerBackbone(nn.Module):
    ...
    def forward(self, samples):
        ...
        vid_frames = rearrange(samples,'b t c h w -> b c t h w')
        ...
        layer_outputs = [rearrange(o, 'b c t h w -> b t c h w') for o in layer_outputs]
...
if __name__ == '__main__':
    x = torch.randn([2, 3, 3, 320, 320])
    mask = torch.zeros([2, 3, 320, 320])
    sample = NestedTensor(x, mask)
    model = VideoSwinTransformerBackbone(False, None, False, 'train')
    out = model(x)
    for o in out:
        print(o.shape)