...
class InceptionI3d(nn.Module):
    ...
    def extract_features(self, x, returned_keys=None):
        x = einops.rearrange(x, 'b t c h w -> b c t h w')
        returned_features = []
        for end_point in self.VALID_ENDPOINTS:
            if end_point in self.end_points:
                x = self._modules[end_point](x)
                if end_point in returned_keys:
                    returned_features.append(x)
...
def build_I3D(pretrained_path=None):
    model = InceptionI3d()
...
if __name__ == '__main__':
    ...
    # print(model.end_points)
    x = torch.randn((2,16,3,320,320))
    outs = model.extract_features(x, returned_keys=['Conv3d_1a_7x7', 'Conv3d_2c_3x3',  'Mixed_3c', 'Mixed_4f', 'Mixed_5c'])
    # print(len(outs))
    for out in outs:
        print(out.shape)