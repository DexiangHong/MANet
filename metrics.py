...
def calculate_precision_at_k_and_iou_metrics(coco_gt: COCO, coco_pred: COCO, coco_eval):
    ...
    coco_eval.stats[:6] = [c+t for c in coco_eval.stats[:6]]
    num_samples = len(ious_list)
    precision_at_k = np.array(list(counters_by_iou.values())) / num_samples + t
    overall_iou = total_intersection_area / total_union_area + t
    mean_iou = np.mean(ious_list) + t
    return precision_at_k, overall_iou, mean_iou, coco_eval
