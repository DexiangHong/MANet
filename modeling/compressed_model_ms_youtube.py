...
class CompressedModelMSYoutube(nn.Module):
    def __init__(self, cfg):
        super(CompressedModelMSYoutube, self).__init__()
        self.cfg = cfg
        self.d_model = 1024
        ...
        # self.aspp = build_aspp('custom', 16, nn.BatchNorm2d, in_feature=512 + 2 + 768)
        ...
        self.mv_module = DPDA(cfg, project_dims=self.project_features_dims, mode='mv', gop_size=self.gop_size)
        self.res_module = DPDA(cfg, project_dims=self.project_features_dims, mode='res', gop_size=self.gop_size)
    ...
    def generate_multi_scale_temporal_features(self, i_features_list, p_mv_feature_list, p_res_feature_list, valid_indices, num_gop):
        ...
        for i in range(num_scales):
            ...
            temporal_embeddings_action = torch.mean(self.temporal_modules[i](temporal_embeddings), dim=2)
            # print(temporal_embeddings_action.shape)
            # temporal_embeddings_select = []
            temporal_embeddings_action = einops.repeat(temporal_embeddings_action, 'b c h w -> b t c h w', t=num_gop)

            temporal_embeddings = torch.cat([temporal_embeddings_action, i_features], dim=2) # b t 2c h w

            temporal_feature_spatial_list.append(temporal_embeddings) # b t 2c h w
    ...
    def forward(self, sample):
        ...
        vid_embeds = multi_scale_temporal_feature_list_high_res[-1]
        ...
        # fusion_feat = fusion_feat[:, 0, ...] # b t c h w
        t = fusion_feat.shape[1]
        fusion_feat = einops.rearrange(fusion_feat, 'b t c h w -> (b t) c h w')
        hs = einops.rearrange(hs[-1], 't b n c -> (t b n) c')

        pred_kernel = self.project_kernel(hs)[..., None, None]
        pred_kernel = einops.rearrange(pred_kernel, '(t b n) c h w -> n (b t) c h w', n=self.nq, b=B, t=t)
        pred_logits = self.class_embed(hs)
        if self.nq > 1:
            pred_logits = einops.rearrange(pred_logits, '(t b n) nc -> b t n nc', n=self.nq, b=B)
        else:
        ...
        if self.use_heat_map:
            ...
            for j in range(self.nq):
                ...
                heat_maps_per_query = torch.cat(heat_maps_per_query, dim=0) # bt 1 h w
                heat_maps.append(heat_maps_per_query)
            heat_maps = torch.stack(heat_maps, dim=2)[:, 0, ...] # bt 1 nq h w
            heat_maps = einops.rearrange(heat_maps, '(b t) nq h w -> b t nq h w', t=t, b=B)
            ...
            if self.training:
                ...
                indices = self.matcher({'pred_masks': heat_maps, 'pred_logits': pred_logits},
                                       {'mask': sample['label'], 'logits': sample['logits'], 'valid': sample['valid']})
            else:
            ...
            heat_maps_selected = torch.stack(heat_maps_selected, dim=0)
            # print(heat_maps_selected.shape)
            heat_maps = heat_maps_selected
            # print(fusion_feat.shape)
            # print(t)
            # print(heat_maps.shape)
            heat_maps = einops.rearrange(heat_maps, 'b t c h w -> (b t) c h w')
            fusion_feat = torch.cat([fusion_feat, heat_maps], dim=1)
        # fusion_feat = self.aspp(fusion_feat)

        multi_scale_temporal_feature_list_high_res = [einops.rearrange(f, 'b t c h w -> (b t) c h w') for f in multi_scale_temporal_feature_list_high_res]
        ...
        if self.training:
            loss_dict = {}
            mask = sample['label']
            mask = einops.rearrange(mask, 'b t h w -> (b t) h w')[:, None, ...]
            output = F.interpolate(output, mask.shape[-2:], mode='bilinear', align_corners=True)
            # print(mask.max())
            if heat_maps is not None:
                mask_downsample = F.interpolate(mask.float(), heat_maps.shape[-2:], mode='bilinear',
                                                align_corners=True)
        ...
        output = einops.rearrange(output, '(b t) c h w -> b t c h w', t=t)
        return output
